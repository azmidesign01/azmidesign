...
function setupdaftarMember(user, element, status) {
   var price = globalSetting['Harga Member'] || 'Rp350.000';
   var produk = globalSetting['Judul Produk Member'];
   var linkproduk = globalSetting['Link Produk Member'];
   var rekening = "BCA - 5965010707 a/n Safri Azmi";
   var redirectURL = "https://www.azmidesign.id/p/upload-produk.html"; // <- Ganti sesuai halaman kamu

   var html = `
    <p><b>Gabung Jadi Member Full Akses</b></p>

<p>Gabung Member hanya 
    <span class="box-price-abu"><b>${price}</b></span> 
    atau beli produk 
    <a href='${linkproduk}' target='_blank' class='link-produk'>
      <b>${produk}</b>
    </a> 
    Langsung bisa download file tanpa batas!
  </p>


    <form class='form-daftar-member box-form'>
      <input id='data-name' type='hidden' value='${user.displayName || ""}'/>
      <input id='data-nomor' type='hidden'/>
      <input id='data-email' type='hidden' value='${user.email || ""}'/>
      <button class='button' style='margin:auto' role='button' type='submit'>
        ...
        <path d='M480.768,87.936l-56.704-56.704c-5.674-5.585-11.957-10.515-18.731-14.699V64
        ...
        C464.214,511.93,511.93,464.215,512,405.334V163.35C512.08,135.049,500.833,107.893,480.768,87.936z
        M256,384c-47.128,0-85.333-38.205-85.333-85.333s38.205-85.333,85.333-85.333s85.333,38.205,85.333,85.333
        S303.128,384,256,384z'></path>
        <path d='M213.333,128h85.333c35.346,0,64-28.654,64-64V1.366
        c-4.638-0.756-9.32-1.212-14.016-1.365H149.333v64C149.333,99.346,177.987,128,213.333,128z'></path></g>
        </svg><span>Daftar Sekarang</span>
      </button>
    </form>
   `;
   ...
   document.querySelector("#html-daftar-member .form-daftar-member").addEventListener("submit", function(event) {
      ...
      var checkoutWA = "";
      checkoutWA += "=========================%0ADAFTAR MEMBER%0A=========================%0A";
      checkoutWA += "Halo Kak, Saya Mau Daftar Member.%0A%0A";
      checkoutWA += "Nama: " + nama + "%0A";
      checkoutWA += "Nomor HP: " + nomor + "%0A";
      checkoutWA += "Email: " + email + "%0A";
      checkoutWA += "Harga: " + price + "%0A";
      checkoutWA += "Pembayaran: " + rekening + "%0A";
	  checkoutWA += "%0ASetelah transfer, kirim bukti ke sini ya kak 🙏";
...
function setupListberita(status) {
     ...
     function renderTable(data) {
      if (status == "admin"){
        let html = `<section class='content-list'>
        ...
        let foundBerita = false;
          data.forEach(function (doc) {
			...
			var file = data.file;
            var checkimage = data.image;
            if (checkimage === undefined || "") {
               var image = globalSetting['Url Thumbnail'];
            } else {
            ...
            if (searchQuery == '' || searchjudul.indexOf(searchQuery) != -1) {
              foundBerita = true;
              html += `
                 <article class='data-berita list-data' data-uid='${uid}'>
                    <button class='checklist'><input type='checkbox'/></button>
                    <div class='list-content'>
                        <div class='thumbnail'>
                           <img src='${image}' alt='${judul} width='480' height='320'/>
                        </div>
                        <div class='post-summary'>
                           <h3 class='title-post'>${judul}</h3>
                           <button class='button link-button-download' data-file='${file}'>Download</button>
                        </div>
                    </div>
                 </article>
              `;
        ...
        updateButtons(data.size);
          ...
          // status member
          let html = `<section id="content-wrap"><div class='content'>`;
          let foundBerita = false;
          data.forEach(function (doc) {
            var uid = doc.id;
            var data = doc.data();
            ...
            if (searchQuery == '' || searchjudul.indexOf(searchQuery) != -1) {
              foundBerita = true;
              html += `<article class='post-outer'>
                     <div class='post-content'>
                       <div class='img-thumbnail'>
                          <img src='${image}' width='480' height='320' alt='${judul}'/>
                       </div>
                       <div class='post-summary-wrap'>
                         <h3 class='post-title entry-title'><a href='${file}' target='_blank' rel='nofollow'>${judul}</a></h3>
                         <div class='post-snippet-wrap'>
                             <div class='detail-preview'><a href='${file}' target='_blank'>Download</a></div>
                         </div>
                       </div>
                       <div class='clear'></div>
                     </div>
